...
bool Foam::UPstream::init(int& argc, char**& argv, const bool needsThread)
{
    ...
    const char* eofFoamRanksEnv = std::getenv("EOF_OF_RANKS");
    const char* eofFoamStartEnv = std::getenv("EOF_OF_RANKS_START");

    const int nFoamRanks = eofFoamRanksEnv ? std::atoi(eofFoamRanksEnv) : numprocs;
    const int foamStart = eofFoamStartEnv ? std::atoi(eofFoamStartEnv) : 0;

    DynamicList<int> foamRanks;
    for (int i = 0; i < nFoamRanks; ++i)
    {
        foamRanks.append(foamStart + i);
    }

    MPI_Group worldGroup, foamGroup;
    MPI_Comm_group(MPI_COMM_WORLD, &worldGroup);
    MPI_Group_incl(worldGroup, foamRanks.size(), foamRanks.cdata(), &foamGroup);
    ...
    if (PstreamGlobals::MPI_COMM_FOAM == MPI_COMM_NULL)
    {
        FatalErrorInFunction
            << "Failed to create EOF OpenFOAM communicator"
            << Foam::abort(FatalError);
    }
...
)
{
    ...
    if (parentIndex == -1)
    {
        ...
        PstreamGlobals::MPICommunicators_[index] =
            PstreamGlobals::MPI_COMM_FOAM == MPI_COMM_NULL
        ? MPI_COMM_WORLD
        : PstreamGlobals::MPI_COMM_FOAM;
