...
!------------------------------------------------------------------------------
     ...
     TYPE(Variable_t), POINTER :: VelVar, BVar
     REAL(KIND=dp), POINTER :: Velocity(:), Bfield(:)
...
!------------------------------------------------------------------------------
     ...
     VelVar => VariableGet( Solver % Mesh % Variables, 'Flow Velocity' )
     IF(.NOT.ASSOCIATED(VelVar)) CALL Fatal('MHDSolve','Flow Velocity not found')
     Velocity => VelVar % Values
     
     BVar => VariableGet( Solver % Mesh % Variables, 'Magnetic Flux Density' )
     IF(.NOT.ASSOCIATED(BVar)) CALL Fatal('MHDSolve','Magnetic Flux Density not found')
     Bfield => BVar % Values
...
!------------------------------------------------------------------------------
    ...
    REAL(KIND=dp) :: vel(3), B(3), UxB(3), CUxB(3)
...
!------------------------------------------------------------------------------
        ...
        B   = 0.0_dp
        DO i=1,DIM
            vel(i) = SUM( Velocity(DIM*(NodeIndexes(1:n)-1)+i) * Basis(1:n) )
            B(i)   = SUM( Bfield  (DIM*(NodeIndexes(1:n)-1)+i) * Basis(1:n) )
        END DO

        UxB = 0.0_dp
        UxB(1) = vel(2)*B(3) - vel(3)*B(2)
        UxB(2) = vel(3)*B(1) - vel(1)*B(3)
        UxB(3) = vel(1)*B(2) - vel(2)*B(1)

        CUxB = 0.0_dp
        IF (GetCondAtIp) THEN
            CUxB(1:DIM) = CondAtIp * UxB(1:DIM)
        ELSE
        DO i=1,DIM
            DO j=1,DIM
                CUxB(i) = CUxB(i) + SUM(Conductivity(i,j,1:n) * Basis(1:n)) * UxB(j)
            END DO
        END DO
        END IF
          ...
          IF( CalculateHeating .OR. CalculateCurrent .OR. CalculateNodalHeating ) THEN
            HeatingDensity = HeatingDensity + &
                s * SUM( Grad(1:DIM) * EpsGrad(1:DIM) ) 
            DO j = 1,DIM
                Current(j) = Current(j) + (CUxB(j) - EpsGrad(j)) * s
            END DO
...
!------------------------------------------------------------------------------
       ...
       REAL(KIND=dp) :: UxB(3), vel(3), B(3), CU(3)
       INTEGER :: j
...
!------------------------------------------------------------------------------
         DO p=1,N
           ...
           END DO
            Force(p) = Force(p) + S*L*Basis(p)   ! original source
            DO i=1,DIM
                Force(p) = Force(p) + S * dBasisdx(p,i) * CU(i)
            END DO
         END DO
!------------------------------------------------------------------------------
       END DO
